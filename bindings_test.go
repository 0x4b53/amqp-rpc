...
func TestSkipQueueDeclare(t *testing.T) {
	...
	s = NewServer(testURL)
	s.Bind(DirectBinding(queueName, handler).With(func(b *HandlerBinding) {
		// Something different, normally this fails bu since we set
		// SkipQueueDeclare it will skip declaring the queue so it won't care.
		b.QueueArgs["x-expires"] = 33
		b.SkipQueueDeclare = true
	}))
